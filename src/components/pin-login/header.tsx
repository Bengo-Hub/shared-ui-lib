...
export function PinLoginHeader({
  ...
  return (
    <div className={cx('relative z-10 px-4 sm:px-8 pt-4 sm:pt-8 pb-3.5 sm:pb-5 shrink-0', className)}>
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] sm:text-sm font-black uppercase tracking-[0.2em] sm:tracking-[0.3em] text-white/75 truncate">
            {serviceName}
          </p>
          <h1 className="mt-0.5 sm:mt-1 text-xl sm:text-3xl md:text-4xl font-black text-white uppercase tracking-tight leading-tight truncate">
            {tenantName}
          </h1>
          {(outletName || showSwitchOutlet) && (
            <div className="flex items-center gap-2 flex-wrap mt-1 sm:mt-2">
              {outletName && (
                <span className="text-sm sm:text-lg font-bold text-white/90 truncate max-w-[10rem] sm:max-w-[16rem]">
                  {outletName}
                </span>
              )}
              {isHQ && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold bg-white/15 text-white ring-1 ring-inset ring-white/20">
                  <Building2 className="h-2.5 w-2.5 sm:h-3 sm:w-3" />HQ
                </span>
              )}
